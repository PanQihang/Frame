            ...
            dataTable.DataTable({
                ...
                "columnDefs": [{
                    "render" : function(data, type, row) {
                        var a = "";
                        a += "<button type='button' class='btn btn-primary' onclick='showEditStudent(\""+row.student_id+"\")' data-toggle='modal' data-target='#myModal5' title='编辑用户' data-toggle='dropdown' style='margin-right:15px; margin-bottom: -1px;'><i class='fa fa-pencil-square-o'></i>&nbsp;编辑</button>"
                        a += "<button type='button' class='btn btn-primary' onclick='deleteStudent(\""+row.student_id+"\")' title='删除用户' data-toggle='dropdown' style='margin-right:15px; margin-bottom: -1px;'><i class='fa fa-user-times'></i>&nbsp;删除</button>"
    ...
    $.ajax({
        ...
        success:function (result){
            ...
            if(role==1)
            {
                ...
                else if(Html_id=='dialogMajorNameB')
                {
                    removeOption('dialogClassNameB');
                    $("#dialogClassNameB").append(SelectInfo);
...
function saveStudentListB(){
    if(!checkStudentList(NewStudentList)) //检查学生账号集的数据（学号、姓名）是否符合规范
        return ;
    NewStudentList.class_id=$('#dialogClassNameB').find('option:selected').val(); //临时将学生账号集的班级所属添加上
    NewStudentList.college_id=$('#dialogMajorNameB').find('option:selected').val(); //临时将学生账号集的学院所属添加上
        $.ajax({
            type: "POST",
            ...
            success:function (result){
                ...
                if(result.result == "succeed"){
                    ...
                    //关闭模态窗口
                    $('#myModal5').modal('hide');
                    $('#myModal5B').modal('hide');
                    $('#myModal5C').modal('hide');
...
function checkStudentInf(t, acType){
    var XTypeRule = /^[0-9]*$/;
    if(!XTypeRule.test(t.account)  || t.account.length>20 || t.account.length<1) {
        swal('学号(' + t.account + ')格式不正确', '请确保为长度不大于20的非空数字', 'error');
        return false;
    }else if(t.name.length>20 || t.name.length<1){
        swal('姓名(' + t.name + ')格式不正确', '请确保为长度不大于20的非空字符串', 'error');
        return false;
    }else if('checkClassId'==acType){
        if(role==1)
        {
            if(t.college_id=='' || t.college_id == '0'){
                swal('未指定学院', '请指定所属学院', 'error');
                return false;
            }
        }
        if(t.class_id=='' || t.class_id == '0'){
