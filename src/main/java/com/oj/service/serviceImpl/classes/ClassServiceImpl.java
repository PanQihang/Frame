package com.oj.service.serviceImpl.classes;

import com.oj.entity.classes.Class;
import com.oj.mapper.classes.ClassMapper;
...
public class ClassServiceImpl implements ClassService {
    ...
    {
        return mapper.getCollegeSelectInfo();
    }
...
//    }
    ...
    {
        //若当前班级名已经存在，则抛出班级已存在的异常
        if(mapper.getClassByName(clas.getName(),clas.getCollege_id()).size()>0){
            throw new Exception("当前班级名称已存在!");
        }else{
            //若班级id为空，为保存
            if (StringUtils.isEmpty(clas.getClass_id())){
                    mapper.save(clas);
            }else{
                mapper.update(clas);
            }
